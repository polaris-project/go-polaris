...
func (client *Client) HandleReceiveBestTransactionRequest(stream inet.Stream) {
	...
	writer.Write(append(bestTransaction.Bytes(), []byte(";DELIM")...)) // Write best transaction
}
...
func (client *Client) HandleReceiveTransactionRequest(stream inet.Stream) {
	...
	readWriter.Write(append(transaction.Bytes(), []byte(";DELIM")...)) // Write transaction bytes
}
...
func (client *Client) HandleReceiveConfigRequest(stream inet.Stream) {
	...
	writer.Write(append((*client.Validator).GetWorkingConfig().Bytes(), []byte(";DELIM")...)) // Write config bytes
}
...
func (client *Client) HandleReceiveGenesisHashRequest(stream inet.Stream) {
	...
	writer.Write(append((*client.Validator).GetWorkingDag().Genesis.Bytes(), []byte(";DELIM")...)) // Write genesis hash
}
...
func (client *Client) HandleReceiveTransactionChildHashesRequest(stream inet.Stream) {
	...
	if err == nil { // Check no error
		...
		readWriter.Write(append(summarizedChildHashes, []byte(";DELIM")...)) // Write child hashes
	}
}
